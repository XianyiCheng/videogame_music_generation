...
def crbm_inference(v, W, bh):
    conv_h = tf.nn.convolution(xk.reshape([1,num_timesteps,span,0]),W,padding = 'VALID',strides = [conv_strides,1])
    prob_h = conv_h + bh

    return prob_h

def crbm_reconstruct(h,W,bv):
    deconv_v = tf.nn.conv2d_transpose(hk,W,output_shape = [1,num_timesteps,span,1],strides = [1,conv_strides,1,1],padding = 'VALID')
    prob_v = deconv_v + bv
...
def gibbs_sample(x, W, bv, bh, k):
    ...
    def gibbs_step(count, k, xk):
        ...
        hk = sample(tf.sigmoid(prob_hk)) #Propagate the visible values to sample the hidden values
        prob_xk = crbm_reconstruct(hk,W,bv)
        xk = sample(tf.sigmoid(prob_xk)) #Propagate the hidden values to sample the visible values
        xk = xk.reshape([num_timesteps,span])
        return count+1, k, xk
